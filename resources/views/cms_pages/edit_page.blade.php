...
<div class="container">	
    ...
    @if(auth()->check()  && auth()->user()->id==$answer->author_id)
	<div class="row">
	 <div class="col-md-12">
    <div class="topics-list">
	<form method="post" action="{{ url("/answers/{$answer->id_answer}") }}"  enctype="multipart/form-data">
         @csrf
        @method('PUT')
        <label for="content">Update Answer : {{$answer->author_id}}</label>
		...
		<input type="hidden" id="author_id" name="author_id" value="{{ auth()->user()->id }}" required> 
